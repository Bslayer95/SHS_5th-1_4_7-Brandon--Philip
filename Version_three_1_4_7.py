...
def frame(original_image, color, percent_of_side):
    ...
    if use_decorative_frame: 
        frame_pic = PIL.Image.open(os.path.join(os.getcwd(), 'frame.jpg')) #This hollows out the frame and masks it over the image 
        frame_pic = frame_pic.resize(result.size)
        result.paste(frame_pic, (0,0), mask=frame_mask)
    else:
        result.paste(frame_mask, (0,0), mask=frame_mask)
     
        directory= os.getcwd() # I cannot get the logo thing to paste in correctly and I have no idea why?
        
        symbol_file= os.path.join(directory, 'logo.png') #takes symbol and inserts it on the lower right corner of image and frame
        symbol_img= PIL.Image.open(symbol_file)
        symbol_small= symbol_img.resize((thickness, thickness))
        
        result.paste(symbol_small, (width-thickness, height-thickness), mask= symbol_small)
...
def frame_logo_images(directory=None, color=(255,0,0), width=0.10):
